...
import 'package:fluttergithub/common/constant/ignore.dart';
...
import 'package:fluttergithub/common/util/navigator_utils.dart';
...
class _LoginRouteState extends State<LoginRoute> {
  ...
  void _onLogin() async {
    ...
    if ((_formKey.currentState as FormState).validate()) {
      ...
      UserBean user;
      try {
        //需要提前获取token才行 不然是登录不了的
        String code = await NavigatorUtils.goLoginWebView(context,
            "https://github.com/login/oauth/authorize?client_id=${Ignore.clientId}&state=app&redirect_uri=https://localhost:8080/authed", "Oauth Login");

        print(code);
        user = await NetApi(context)
            .authLogin(code); //实际是调用oauth进行登录
